...
import numpy as np
import sklearn.preproessing as prep
import tensorflow as tf
from tensorflow.example.tutorials.mnist import input_data

# weights initialize function
...
class AdditiveGaussianNoiseAutoencoder(object):
    ...
    optimizer = tf.train.AdamOptimizer(), scale = 0.1):
        ...
        self.reconstruction = tf.add(tf.matmul(self.hid['w2']), self.weights['b2'])
    ...
    def partial_fit(self, X):
        cost, opt = self.sess.run((self.cost, self.optimizer),
            feed_dict = {self.x: X, self.scale: training_scale})
        return cost
    ...
    def generate(self, hidden = None):
        if hidden is None:
            hidden = np.radnom.normal(size = self.weights["b1"])
...
def standard_scale(X_train, X_test):
    preprocessor = prep.StandardScaler().fix(X_train)
